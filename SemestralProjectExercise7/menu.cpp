	...
	do {
		...
		case 3:
			if (clientDatabaseSize > 0)
				printFromTemplate(clientsPtr, clientDatabaseSize);
			break;
		case 4:
			if (clientDatabaseSize > 0) {
				...
				cin >> clientIndex;
				if (clientIndex >= 0)
					deleteFromTemplate(clientsPtr, clientDatabaseSize, clientIndex);
			}
			break;
		case 5:
		...
		case 14:
			if (employeeDatabaseSize > 0)
				printFromTemplate(employeesPtr, employeeDatabaseSize);
			break;
		case 15:
			if (employeeDatabaseSize > 0) {
				...
				cin >> employeeIndex;
				if (employeeIndex >= 0)
					deleteFromTemplate(employeesPtr, employeeDatabaseSize, employeeIndex);
			}
			break;
		case 16:
			if (employeeDatabaseSize > 0) {
			...
			break;
		case 17:
			deleteFromTemplate(clientsPtr, clientDatabaseSize);
			deleteFromTemplate(employeesPtr, employeeDatabaseSize);
			_exit = true;
			break;
		default:
