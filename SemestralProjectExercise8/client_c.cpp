...
Client::Client(string nip, string companyName, string companyLocation, size_t contractValue, string contactPersonName, string contactPersonSurname, string contactPersonPhoneNumber, string contactPersonMail) : id{ rand() % 100 + 1 } {
	this->nip = nip;
...
void Client::setTicketIsDone(size_t index, bool c, string x) {
	if (index > numberOfTasks)
		return;
	taskList[index]->setIsDone(c);
	taskList[index]->setClosedBy(x);
}